...
### Sample ERC721 Contracts

This includes a very simple sample ERC721 for the purposes of demonstrating integration with the [OpenSea](https://opensea.io) marketplace.
...
yarn hardhat run scripts/deploy.js rinkeby
```
...
- Do you have any conflicts that result from globally installed node packages? If yes, try `yarn remove -g hardhat; yarn`
...
```sh
yarn add --dev hardhat @nomiclabs/hardhat-ethers ethers @nomiclabs/hardhat-waffle ethereum-waffle chai

```

You can also debug just the compile step by running `yarn hardhat compile`.
